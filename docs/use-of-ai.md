...
For this reason, I have taken an approach where if AI is not mentioned in the test/assessment scope, either making it a requirement or excluding it, then it becomes a candidate choice, the same way adding a dev dependency would be.
...
**Claude Desktop:** Was great for planning exercises (see approach), however, it reached its usage limit very quickly.
...
**Grok:** With only a web client, Grok proved very helpful when problem-solving in a chat interface was needed, but without local filesystem access, it sometimes needed a lot of copy-pasting back and forth, so usage is limited.

**Claude Code:** Of all the coding changes, Claude Code proved the fastest and best at problem-solving, provided you used the planning feature. However, even with a Pro license, it was easy to reach usage limits.

## Multi-Agent

The git working tree feature was used to checkout a maximum of 2 branches at a time. Then two agents would be used at a time on different tasks. For example, Claude Desktop on one branch and Gemini CLI on another, or Gemini for both.
...
The most this multi-agent approach was extended to was two cli agents, one in each branch, and a Grok chat. More than this, I don't think it is feasible as the cognitive load becomes too high.
